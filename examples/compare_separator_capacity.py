"""
Compare simulation results with different platelet_separator capacities
"""
...
def create_config(separator_capacity):
    """Create configuration with specified platelet_separator capacity."""
    return {
        "simulation": {
            "duration": 43200,
            "random_seed": 42,
...
def run_comparison():
    """Run simulations with different platelet_separator capacities and compare."""
    print("=" * 100)
    print("PLATELET SEPARATOR CAPACITY COMPARISON")
    print("=" * 100)
    ...
    for capacity in [1, 4]:
        print(f"\n{'='*100}")
        print(f"Running simulation with platelet_separator capacity = {capacity}...")
        print(f"{'='*100}")
    ...
    print(f"\n{'Metric':<30} {'Capacity 1':<25} {'Capacity 4':<25} {'Difference':<20}")
    print("-" * 100)
    ...
    for metric_name, metric_key in metrics.items():
        if callable(metric_key):
            val_1 = metric_key(results[1]['summary'])
            val_4 = metric_key(results[4]['summary'])
        else:
            val_1 = results[1]['summary'][metric_key]
            val_4 = results[4]['summary'][metric_key]
        
        diff = val_4 - val_1
        if isinstance(val_1, float):
            print(f"{metric_name:<30} {val_1:<25.2f} {val_4:<25.2f} {diff:+.2f} ({diff/val_1*100:+.1f}%)")
        else:
            print(f"{metric_name:<30} {val_1:<25,} {val_4:<25,} {diff:+,} ({diff/val_1*100:+.1f}%)")
    
    # Device states comparison
    print(f"\n{'Device Final States':<30} {'Capacity 1':<25} {'Capacity 4':<25}")
    print("-" * 100)
    
    # Convert list to dict for easier lookup
    states_1 = {d['device_id']: d['final_state'] for d in results[1]['device_states']}
    ...
    for device_id in ["centrifuge", "platelet_separator", "quality_check", "final_inspection"]:
        state_1 = states_1.get(device_id, "N/A")
        state_4 = states_4.get(device_id, "N/A")
        print(f"{device_id:<30} {state_1:<25} {state_4:<25}")
    ...
    time_diff = results[1]['summary']['simulation_time_seconds'] - results[4]['summary']['simulation_time_seconds']
    event_diff = results[4]['summary']['total_events'] - results[1]['summary']['total_events']
    
    print(f"\n✓ Increasing platelet_separator capacity (1→4):")
    print(f"  • Reduces completion time by {time_diff:.1f}s ({time_diff/60:.1f} minutes)")
    print(f"  • Changes total events by {event_diff:,} ({event_diff/results[1]['summary']['total_events']*100:+.1f}%)")
    
    if time_diff > 0:
        print(f"  • Throughput improvement: {(time_diff/results[1]['summary']['simulation_time_seconds'])*100:.1f}%")
        print(f"\n💡 RESULT: platelet_separator IS a bottleneck! Increasing capacity improves throughput.")
    else:
        print(f"  • No throughput improvement")
        print(f"\n💡 RESULT: platelet_separator is NOT the bottleneck. Look at other single-capacity devices:")
        print(f"     - quality_check (capacity 1)")
        print(f"     - final_inspection (capacity 1)")
    
    # Save detailed results
    with open("comparison_separator_results.json", "w") as f:
        json.dump({
            "capacity_1": results[1],
            "capacity_4": results[4]
        }, f, indent=2)
    
    print(f"\n💾 Detailed results saved to: comparison_separator_results.json")
    print("=" * 100)
